...
class Renderer:
    ...
    def _render_impl(self, res,
        ...
        # Calculate final W.
        w = torch.stack([all_ws[seed] * weight for seed,
                        weight in w0_seeds]).sum(dim=0, keepdim=True)
        ...
        if cache_key not in self._net_layers:
            if layer_name is not None:
                torch.manual_seed(random_seed)
                _out, layers = self.run_synthesis_net(
                    G.synthesis, w, **synthesis_kwargs)
        ...
        if fft_show:
            ...
            sig = sig * torch.kaiser_window(
                sig.shape[1], periodic=False, beta=fft_beta, device=self._device)[None, :, None]
            sig = sig * torch.kaiser_window(
                sig.shape[2], periodic=False, beta=fft_beta, device=self._device)[None, None, :]
            fft = torch.fft.fftn(sig, dim=[1, 2]).abs().square().sum(dim=0)
            fft = fft.roll(shifts=[fft.shape[0] // 2,
                           fft.shape[1] // 2], dims=[0, 1])
    ...
    def run_synthesis_net(net, *args, capture_layer=None, **kwargs):  # => out, layers
        ...
        def module_hook(module, _inputs, outputs):
            ...
            outputs = [out for out in outputs if isinstance(
                out, torch.Tensor) and out.ndim in [4, 5]]
